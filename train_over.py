...
train_over.py
=============
Training script for a starspot *detection* classifier applied to overcontact
(W UMa-type / EW) eclipsing binary light curves.

The task is binary: given a phase-folded light curve, predict whether the
primary or secondary component hosts a starspot that causes a measurable
asymmetry in the observed photometry.
...
weights) whose global-average-pooling output (512-d) is fed into a two-layer
MLP classifier with dropout regularisation.  This lightweight head is
sufficient given that the three input channels already encode physically
motivated, spot-sensitive representations.

Input data format
-----------------
A CSV file ``selected_data_over.csv`` in the working directory with the
...
-----------------------------------------------------------------
    Ch 0 – Gradient map    : first derivative of flux, highlights slope
                             changes caused by spot-induced asymmetry.
    Ch 1 – Wavelet detail  : Daubechies db1 level-1 detail coefficients,
                             sensitive to high-frequency flux variations.
    Ch 2 – Adaptive stretch: 1st–99th percentile contrast stretch, preserves
                             subtle out-of-eclipse modulation.
...
- Cross-entropy loss.
- Early stopping: patience = 5 epochs, minimum improvement δ = 1 × 10⁻⁴.
- Best checkpoint saved to ``best_model_spots.pth``.
- Per-epoch metrics appended to ``training_history_spots.csv``.
...
Outputs
-------
best_model_spots.pth         – state dict of the best validation checkpoint.
training_history_spots.csv   – per-epoch loss and accuracy log.
training_stats_dashboard.png – 2-panel plot (loss and accuracy curves).

Usage
-----
    python train_over.py
...
class SpotDetectionDataset(Dataset):
    ...
    Unlike the binary morphology classifier (``train_binary.py``), the channel
    encoding here is designed to amplify subtle, low-amplitude flux asymmetries
    caused by starspots rather than the gross shape differences between contact
    and detached morphologies.
    ...
    def _to_3channel(self, phases: np.ndarray, fluxes: np.ndarray) -> torch.Tensor:
        ...
        Channel 0 – Gradient map
            The first derivative of flux is normalised to [0, 1] and plotted
            as a scatter image.  Starspots introduce asymmetric slopes in the
            out-of-eclipse regions that are invisible in raw flux images.
        ...
        Channel 1 – Wavelet detail coefficients
            ...
            variations caused by spot crossing events.

        Channel 2 – Adaptive contrast stretch
            The flux is clipped to the [1st, 99th] percentile range and
            mapped to [0, 1].  This suppresses outliers while preserving
            low-amplitude, broad modulations characteristic of spots on the
            stellar limb.
...
class MultiScaleSpotNet(nn.Module):
    ...
    extractor.  Its fully-connected head is replaced with an ``nn.Identity``
    so that the 512-d global-average-pooling vector is passed directly to a
...
def train_model() -> None:
    """Fine-tune MultiScaleSpotNet on the starspot detection task.

    Workflow
    --------
    1. Load and shuffle ``selected_data_over.csv``; split 80 / 20.
    ...
    4. Run up to 100 epochs with early stopping
       (patience = 5, min δ = 1 × 10⁻⁴ on val loss).
    5. Save the best checkpoint and log per-epoch metrics to CSV.
    6. Generate and save a 2-panel loss/accuracy dashboard.

    Output files
    ------------
    best_model_spots.pth         – best validation-loss checkpoint.
    training_history_spots.csv   – epoch-level loss and accuracy log.
    training_stats_dashboard.png – loss and accuracy curves.
    """
    LOG_FILE   = "training_history_spots.csv"
    DATA_PATH  = "selected_data_over.csv"
    MODEL_FILE = "best_model_spots.pth"
    PLOT_FILE  = "training_stats_dashboard.png"
